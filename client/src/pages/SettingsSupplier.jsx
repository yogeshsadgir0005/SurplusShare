...
const SettingsSupplier = () => {
  ...
  const [formState, setFormState] = useState({
    email: '', password: '', businessType: 'Restaurant', legalName: '', website: '', contactNumber: '', address: '', city: '', district: '', state: ''
  });
    ...
    const fetchProfile = async () => {
      try {
        ...
        setFormState({
          ...
          businessType: details.businessType || 'Restaurant', legalName: details.legalName || '', 
          website: details.website || '', contactNumber: details.contactNumber || '', 
    ...
    <Layout role="Supplier">
      <div className="max-w-[1000px] mx-auto space-y-8 pb-10">
        <header className="flex flex-col gap-1.5">
          <h1 className="text-[32px] font-extrabold text-[#064e3b] tracking-tight">Supplier Settings</h1>
          <p className="text-[15px] font-medium text-[#4a6b56]">Update your business details, pickup location, and account credentials.</p>
        </header>
        ...
        <form onSubmit={handleUpdate} className="bg-white rounded-[2rem] border border-[#e8f0eb] shadow-[0_8px_30px_rgb(0,0,0,0.03)] overflow-hidden transition-all duration-500 hover:shadow-[0_15px_40px_rgb(0,0,0,0.04)]">
            ...
            <div className="p-6 sm:p-10 space-y-6">
                <h3 className="text-xl font-extrabold text-[#064e3b] border-b border-[#e8f0eb] pb-4 mb-6">Business Profile</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 lg:gap-8">
                    <div className="space-y-1.5">
                      <label className="block text-[12px] font-extrabold text-[#82a38e] uppercase tracking-wider pl-1">Business Type</label>
                      <select name="businessType" value={formState.businessType} onChange={syncInput} className="w-full bg-[#f4f7f4] border-2 border-transparent rounded-full px-5 py-3.5 text-[15px] font-bold text-[#064e3b] outline-none focus:bg-white focus:border-[#10b981]/30 focus:ring-4 focus:ring-[#10b981]/10 transition-all cursor-pointer">
                         <option>Restaurant</option><option>Corporate Cafeteria</option><option>Event Organizer</option><option>Wholesaler</option>
                      </select>
                    </div>
                    <InputField label="Legal Business Name" name="legalName" value={formState.legalName} onChange={syncInput} />
                    <InputField label="Contact Mobile" name="contactNumber" type="tel" value={formState.contactNumber} onChange={syncInput} />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="h-full min-h-[300px] w-full rounded-[2rem] overflow-hidden border-4 border-[#f4f7f4] shadow-sm relative z-10 group">
                      ...
                      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-[#064e3b]/90 backdrop-blur-md text-white px-5 py-2.5 rounded-full text-[12px] font-bold shadow-xl pointer-events-none z-[400] whitespace-nowrap opacity-90 group-hover:opacity-100 transition-opacity">
                         Drag pin to adjust loading dock location
                      </div>
                    </div>
                </div>
            </div>
